...
import { ABIEntity, ABIEntityType, ParamEntity } from "./compilerWrapper.ts";
...
export class FunctionCall {
  ...
  constructor(
    ...
    if (
      binding.lockingScript === undefined &&
      binding.unlockingScript === undefined
    ) {
      throw new Error(
        "param binding.lockingScript & binding.unlockingScript cannot both be empty",
      );
    }
  ...
  genLaunchConfig(txContext?: TxContext): FileUri {
    const constructorArgs: SupportedParamType[] = this.contract.ctorArgs().map(
      (p) => p.value
    );
    ...
    const state: string =
      !AbstractContract.isStateful(this.contract) && this.contract.dataPart
    ...
    if (!result.success) {
      ...
      if (debugUrl) {
        result.error = result.error +
          `\t[Launch Debugger](${
            debugUrl.replace(/file:/i, "scryptlaunch:")
          })\n`;
      }
    }
    return result;
...
export class ABICoder {
  ...
  encodeConstructorCall(
    ...
    const constructorABI =
      this.abi.filter((entity) => entity.type === ABIEntityType.CONSTRUCTOR)[0];
    ...
    const flatteredArgs = flatternCtorArgs(
      cParams.map((p, index) => (Object.assign({ ...p }, {
        value: args[index],
      }))),
      this.resolver.resolverType,
    );
    ...
    flatteredArgs.forEach((arg) => {
      ...
      contract.hexTemplateArgs.set(
        `<${arg.name}>`,
        this.encodeParam(arg.value, arg),
      );
    });
  ...
  encodeConstructorCallFromRawHex(
    ...
    for (let index = 0; index < script.chunks.length; index++) {
      ...
      if (
        offset >= hexTemplate.length && chunk.opcodenum == 106 /*OP_RETURN*/
      ) {
        ...
        if (name.startsWith(`<${contract.contractName}.`)) { //inline asm
          contract.hexTemplateInlineASM.set(
            name,
            bw.toBuffer().toString("hex"),
          );
        } else {
        ...
        if (parseInt(op, 16) != chunk.opcodenum) {
          throw new Error(
            `the raw script cannot match the ASM template of contract ${contract.contractName}`,
          );
        }
    ...
    const ctorArgs: Arguments = cParams.map((param) =>
      deserializeArgfromHex(
        contract.resolver,
        Object.assign(param, {
          value: undefined,
  ...
  encodePubFunctionCall(
    ...
    for (const entity of this.abi) {
      if (entity.name === name) {
        ...
        if (this.abi.length > 2 && entity.index !== undefined) {
          ...
          hex += `${
            bsv.Script.fromASM(int2Asm(pubFuncIndex.toString())).toHex()
          }`;
        }
        return new FunctionCall(name, {
          contract,
          unlockingScript: bsv.Script.fromHex(hex),
    ...
    throw new Error(
      `no public function named '${name}' found in contract '${contract.contractName}'`,
    );
  }
  ...
  encodePubFunctionCallFromHex(
    ...
    const entity =
      this.abi.filter((entity) =>
        entity.type === "function" && entity.name === name
      )[0];
    if (!entity) {
      throw new Error(
        `no public function named '${name}' found in contract '${contract.contractName}'`,
    ...
    flatternArgs.forEach((farg, index) => {
      hexTemplateArgs.set(
        `<${farg.name}>`,
        bsv.Script.fromASM(asmOpcodes[index]).toHex(),
      );
    });
  ...
  encodeParam(arg: SupportedParamType, paramEntity: ParamEntity): string {
    ...
    if (typeofArg === "boolean") {
      ...
      throw new Error(
        `The value of parameter ${paramEntity.name} is unknown type: ${typeofArg}`,
      );
    }
