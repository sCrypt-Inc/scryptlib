...
import { compileAsync, OpCode, StaticEntity } from "./compilerWrapper.ts";
import {
  BasicScryptType,
  HashedMap,
  HashedSet,
...
import { VerifyError } from "./contract.ts";
import { ABIEntity, handleCompilerOutput, LibraryEntity } from "./index.ts";
...
export const DEFAULT_FLAGS =
  ...
  Interp.SCRIPT_ENABLE_SIGHASH_FORKID | Interp.SCRIPT_VERIFY_LOW_S |
  ...
  Interp.SCRIPT_VERIFY_CHECKSEQUENCEVERIFY | Interp.SCRIPT_VERIFY_CLEANSTACK;

export const DEFAULT_SIGHASH_TYPE = bsv.crypto.Signature.SIGHASH_ALL |
...
export function asm2int(str: string): number | string {
  switch (str) {
    ...
    default: {
      ...
      if (
        bn.toNumber() < Number.MAX_SAFE_INTEGER &&
        bn.toNumber() > Number.MIN_SAFE_INTEGER
      ) {
        return bn.toNumber();
      } else {
        return bn.toString();
...
export function literal2ScryptType(l: string): ScryptType {
  ...
  switch (type) {
    ...
    case VariableType.BYTES:
      return isStringLiteral(l)
        ? new String(String.fromUtf8Hex(value as string))
        : new Bytes(value as string);
...
export function asm2ScryptType(type: string, asm: string): ScryptType {
  switch (type) {
    ...
    case VariableType.BYTES:
      return new Bytes(
        (asm == "0" || asm == "OP_0" || asm == "OP_FALSE") ? "" : asm,
      );
    case VariableType.PRIVKEY:
    ...
    default:
      throw new Error(
        `<${type}> cannot be cast to ScryptType, only sCrypt native types supported`,
      );
  }
}
  ...
  switch (type) {
    ...
    case VariableType.RIPEMD160:
      return new Ripemd160(
        chuck.opcodenum == 0 ? "" : chuck.buf.toString("hex"),
      );
    case VariableType.SHA1:
...
export function bytes2Literal(bytearray: number[], type: string): string {
  switch (type) {
    case "bool":
      return BN.fromBuffer(bytearray, { endian: "little" }) > 0
        ? "true"
        : "false";
...
export function getLowSPreimage(
  tx: bsv.Transaction,
  lockingScript: Script,
  inputAmount: number,
  inputIndex = 0,
...
export function num2bin(
  ...
  const padding = n > 0
    ? "00".repeat(paddingLen)
    : "00".repeat(paddingLen - 1) + "80";
...
export function checkStructField(
  s: StructEntity,
  param: ParamEntity,
  ...
  if (isArrayType(expectedType)) {
    if (
      checkArray(arg as SupportedParamType[], param, expectedType, typeResolver)
    ) {
      throw new Error(
        `Member ${param.name} of struct ${s.name} is of wrong type, expected ${param.type}`,
      );
    }
  } else {
    ...
    if (expectedType != realType) {
      throw new Error(
        `Member ${param.name} of struct ${s.name} is of wrong type, expected ${expectedType} but got ${realType}`,
      );
    }
  }
}
...
export function checkSupportedParamType(
  arg: SupportedParamType,
  param: ParamEntity,
  ...
  const error = new Error(
    `The type of ${param.name} is wrong, expected ${finalType} but got ${
      typeOfArg(arg)
    }`,
  );
...
export function arrayTypeAndSizeStr(
  ...
  if (arrayTypeName.indexOf(">") > -1) {
    const elemTypeName = arrayTypeName.substring(
      0,
      arrayTypeName.lastIndexOf(">") + 1,
    );
    const sizeParts = arrayTypeName.substring(
      arrayTypeName.lastIndexOf(">") + 1,
    );
...
export function arrayTypeAndSize(
  ...
  return [
    ...
    arraySizes.map((size) => {
      ...
      if (isNaN(n)) {
        throw new Error(
          `arrayTypeAndSize error type ${arrayTypeName} with sub isNaN`,
        );
      }
...
function checkArray(
  ...
  if (!Array.isArray(args)) {
    return new Error(
      `The type of ${param.name} is wrong, expected ${expectedType} but got ${
        typeOfArg(args)
      }`,
    );
  }
  ...
  if (!args.every((arg) => typeOfArg(arg) === t)) {
    return new Error(
      `The type of ${param.name} is wrong, expected ${expectedType} but not all element types are the same`,
    );
  }

  if (args.length !== arraySizes[0]) {
    return new Error(
      `The type of ${param.name} is wrong, should be ${expectedType}`,
    );
  }
...
export function subscript(index: number, arraySizes: Array<number>): string {
  if (arraySizes.length == 1) {
    ...
    return `[${Math.floor(index / offset)}]${
      subscript(index % offset, subArraySizes)
    }`;
  }
}
...
export function flatternLibrary(
  arg: SupportedParamType,
  name: string,
): Arguments {
  if (Library.isLibrary(arg)) {
...
export function flatternLibraryState(
  arg: SupportedParamType,
  name: string,
): Arguments {
  if (Library.isLibrary(arg)) {
...
export function flatternCtorArgs(
  args: Arguments,
  resolver: TypeResolver,
): Arguments {
...
export function flatternStateArgs(
  args: Arguments,
  resolver: TypeResolver,
): Arguments {
  const args_: Arguments = [];
  args.forEach((arg) => {
    ...
    const finalType = typeInfo.finalType;
    if (isArrayType(finalType)) {
      flatternArray(arg.value as SupportedParamType[], arg.name, finalType)
        .forEach((e) => {
          args_.push({
            name: e.name,
            type: resolver(e.type).finalType,
...
function flatternLibraryParam(
  param: ParamEntity,
  resolver: ScryptTypeResolver,
  property: boolean,
): Arguments {
  const libraryClass = resolver.resolverClass(param.type) as typeof Library;
  const ast = property
    ? libraryClass.libraryAst.properties
    : libraryClass.libraryAst.params;
  return ast.map((p) => {
...
export function flatternParams(
...
export function newCall(
  Cls: typeof AbstractContract,
  args: Array<SupportedParamType>,
): AbstractContract {
...
export function resolveConstValue(node: any): string | undefined {
  ...
  if (node.expr.nodeType === "FunctionCall") {
    if (
      [
        VariableType.PUBKEY,
        VariableType.RIPEMD160,
        VariableType.PUBKEYHASH,
        VariableType.SIG,
        VariableType.SIGHASHTYPE,
        VariableType.OPCODETYPE,
        VariableType.SIGHASHPREIMAGE,
        VariableType.SHA1,
        VariableType.SHA256,
      ].includes(node.expr.name)
    ) {
      value = `b'${
        node.expr.params[0].value.map((a) => intValue2hex(a)).join("")
      }'`;
...
function resolveAliasType(
  ...
  if (isArrayType(type)) {
    ...
    if (isArrayType(elemTypeInfo.finalType)) {
      const [elemTypeName_, sizes_] = arrayTypeAndSizeStr(
        elemTypeInfo.finalType,
      );
      return {
    ...
    const gts = genericTypes.map((t) =>
      resolveAliasType(originTypes, alias, t).finalType
    );
    return {
...
export function createStruct(
  ...
  structClass.structAst.params.forEach((p) => {
    ...
    if (isArrayType(typeInfo.finalType)) {
      ...
      Object.assign(obj, {
        [p.name]: hex2ScryptType(
          typeInfo.finalType,
          opcodesMap.get(`<${param.name}.${p.name}>`),
        ),
      });
    }
  });
...
export function createLibrary(
  ...
  const args = libraryClass.libraryAst.params.map((p) => {
    ...
    if (isArrayType(typeInfo.finalType)) {
      ...
      return createStruct(resolver, {
        name: `${param.name}.${p.name}`,
        type: p.type,
      }, opcodesMap);
    } else if (typeInfo.symbolType === SymbolType.Library) {
      return createLibrary(resolver, {
        name: `${param.name}.${p.name}`,
        type: p.type,
      }, opcodesMap);
    } else {
      return hex2ScryptType(
        typeInfo.finalType,
        opcodesMap.get(`<${param.name}.${p.name}>`),
      );
    }
...
export function createLibraryProperties(
  resolver: ScryptTypeResolver,
  param: ParamEntity,
  opcodesMap: Map<string, string>,
): Record<string, SupportedParamType> {
  const libraryClass = resolver.resolverClass(param.type) as typeof Library;
  ...
  libraryClass.libraryAst.properties.forEach((p) => {
    ...
    if (isArrayType(typeInfo.finalType)) {
      ...
      const library = createDefaultLibrary(resolver, {
        name: `${param.name}.${p.name}`,
        type: p.type,
      });

      library.setProperties(
        createLibraryProperties(resolver, {
          name: `${param.name}.${p.name}`,
          type: p.type,
        }, opcodesMap),
      );
...
export function createArray(
  ...
  if (sizes.length === 1) {
    ...
    for (let index = 0; index < arraylen; index++) {
      ...
      const subArrayType = [
        finalType,
        sizes.slice(1).map((size) => `[${size}]`).join(""),
      ].join("");
      arrays.push(
        createArray(resolver, subArrayType, `${name}[${index}]`, opcodesMap),
      );
    }
  }
...
export function findConstStatic(
  statics: StaticEntity[],
  name: string,
): StaticEntity {
...
export function buildContractState(
  ...
  if (args_.length <= 0) {
    throw new Error(
      "no state property found, buildContractState only used for state contract",
    );
  }
  ...
  for (const arg of args_) {
    if (arg.type == VariableType.BOOL) { //fixed length
      ...
      state_hex += `${
        bsv.Script.fromASM(serializeSupportedParamType(arg.value)).toHex()
      }`;
    }
  }
...
export function buildDefaultStateProps(contract: AbstractContract): Arguments {
  ...
  return contract.stateProps.map((param) =>
    deserializeArgfromState(
      contract.resolver,
      Object.assign(param, {
        value: undefined,
      }),
...
export function deserializeArgfromState(
  resolver: ScryptTypeResolver,
  arg: Argument,
  opcodesMap: Map<string, string>,
): Argument {
...
export function flattenSha256(data: SupportedParamType): string {
  ...
  if (flattened.length === 1) {
    ...
    if (
      (flattened[0] instanceof Bool || flattened[0] instanceof Int) &&
      hex === "00"
    ) {
      hex = "";
    }
    ...
    return bsv.crypto.Hash.sha256(Buffer.from(jointbytes, "hex")).toString(
      "hex",
    );
  }
}
...
export function sortmap(
  map: Map<SupportedParamType, SupportedParamType>,
): Map<SupportedParamType, SupportedParamType> {
...
export function toHashedMap(
  collection: Map<SupportedParamType, SupportedParamType>,
): HashedMap {
...
export function hash160(hexstr: string, encoding?: BufferEncoding): string {
  return bsv.crypto.Hash.sha256ripemd160(Buffer.from(hexstr, encoding || "hex"))
    .toString("hex");
}

// Equivalent to the built-in function `sha256` in scrypt
export function sha256(hexstr: string, encoding?: BufferEncoding): string {
  return bsv.crypto.Hash.sha256(Buffer.from(hexstr, encoding || "hex"))
    .toString("hex");
}
 ...
 */
export function parseAbiFromUnlockingScript(
  ...
  if (!entity) {
    throw new Error(
      `the raw unlocking script cannot match the contract ${contract.contractName}`,
    );
  }
  ...
  if (Array.isArray(a)) {
    ...
    if (Array.isArray(arg0)) {
      if (!a.every((arg) => Array.isArray(arg) && arg.length === arg0.length)) {
        throw new Error(
          `cannot inferr type from [${a}] , not all length of element are the same`,
        );
      }
      ...
      if (!a.every((arg) => typeOfArg(arg) === t)) {
        throw new Error(
          `cannot inferr type from [${a}] , not all element types are the same`,
        );
      }
...
export function resolveGenericType(
  genericTypeMap: Record<string, string>,
  type: string,
): string {
...
export function structSign(structEntity: StructEntity) {
  return `${
    JSON.stringify(
      structEntity.params.reduce((p, v) =>
        Object.assign(p, {
          [v.name]: v.type,
        }), {}),
      null,
...
export function canAssignProperty(libraryAst: LibraryEntity): boolean {
  return libraryAst.params.length === libraryAst.properties.length &&
    libraryAst.params.every((param, index) => {
      return param.name === libraryAst.properties[index].name &&
        param.type === libraryAst.properties[index].type;
    });
}
 ...
 */
export function findSrcInfoV1(
  opcodes: OpCode[],
  opcodesIndex: number,
): OpCode | undefined {
  while (--opcodesIndex > 0) {
    if (
      opcodes[opcodesIndex].pos && opcodes[opcodesIndex].pos.file !== "std" &&
...
export function parseStateHex(
  contract: AbstractContract,
  scriptHex: string,
): [boolean, Arguments] {
  const metaScript = scriptHex.substr(scriptHex.length - 10, 10);
  const version = bin2num(
    metaScript.substr(metaScript.length - 2, 2),
  ) as number;
  const stateLen = bin2num(metaScript.substr(0, 8)) as number;

  const stateHex = scriptHex.substr(
    scriptHex.length - 10 - stateLen * 2,
    stateLen * 2,
  );
  ...
  flatternparams.forEach((param) => {
    if (param.type === VariableType.BOOL) {
      ...
      if (
        param.type === VariableType.INT || param.type === VariableType.PRIVKEY
      ) {
        stateTemplateArgs.set(
          `<${param.name}>`,
          new Int(bin2num(data)).toHex(),
        );
      } else {
        stateTemplateArgs.set(
          `<${param.name}>`,
          bsv.Script.fromASM(data).toHex(),
        );
      }
    }
  });
