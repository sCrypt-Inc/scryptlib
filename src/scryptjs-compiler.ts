...
export function compile(
  ...
  try {
    ...
    // const cmd = `scrypt compile ${settings.asm ? '--asm' : ''} ${settings.ast ? '--ast' : ''} ${settings.debug ? '--debug' : ''} --std -r -o "${currentWorkingDir}"`;
    const cmd = `node "${join(__dirname, '../node_modules/scryptc/scrypt.js')}" compile ${settings.asm ? '--asm' : ''} ${settings.ast ? '--ast' : ''} ${settings.debug ? '--debug' : ''} --std -r -o "${currentWorkingDir}"`;
    console.log(cmd);
    ...
    if (settings.ast) {
      ...
      result.ast = allAst[sourceFileName];
      result.dependencyAsts = Object.keys(allAst)
        .filter(k => k !== sourceFileName)
        .reduce((res, key) => {
          if (key === 'std') {
            res[key] = allAst[key];
          } else {
    ...
    if (settings.asm){
      ...
      if (settings.debug) {
        ...
        result.debugAsm = asmObj.output.map( item => {
          ...
          throw "Compile Failed: Asm output parsing Error!";
        });
...
function getOutputFilePath(baseDir: string, target: 'ast' | 'asm'): string {
