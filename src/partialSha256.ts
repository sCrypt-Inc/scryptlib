...
export function partialSha256(message: Buffer, index: number) {
    ...
    let padded = Buffer.concat([message, suffix]);
    ...
    for (var i = bytesHashed + 1; i < padded.length - 8; i++) {
        padded[i] = 0;
    }
...
export function sha256ByPartialHash(partialHash: string, partialPreimage: string, padding: string): string {
    ...
    let partialHashBuffer = Buffer.from(partialHash, 'hex');
    let padded = Buffer.from(partialPreimage + padding, 'hex');
    ...
    let h0 = [byteToUint32(partialHashBuffer.slice(0, 4)),
...
function toHash(hi: number[]): string {
    let hashBytes = [iToB(hi[0]), iToB(hi[1]), iToB(hi[2]), iToB(hi[3]), iToB(hi[4]), iToB(hi[5]), iToB(hi[6]), iToB(hi[7])]
