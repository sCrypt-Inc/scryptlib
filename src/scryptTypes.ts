...
import { serialize, serializeInt } from "./serializer.ts";
...
export interface ScryptTypeResolver {
  resolverType: (type: string) => TypeInfo;
...
export class ScryptType {
  ...
  constructor(...value: ValueType[]) {
    try {
      ...
      throw new Error(
        `can't construct ${this.constructor.name} from <${value}>, ${error.message}`,
      );
    }
  }
...
export class Int extends ScryptType {
  ...
  checkValue(value: ValueType[]): ValueType {
    ...
    if (!isInteger(value[0])) {
      throw new Error(
        "Only supports integers, should use integer number, bigint, hex string or decimal string: " +
          value[0],
      );
    }
    ...
    if (typeof value[0] == "number" && !isNaN(value[0])) {
      if (!Number.isSafeInteger(value[0])) {
        throw new Error(
          `<${
            value[0]
          }> is not safe integer, should use bigint, hex string or decimal string`,
        );
      }
    }
...
export class SigHashPreimage extends ScryptType {
  ...
  get scriptCode(): string {
    return this.getReader(this._buf.slice(104, this._buf.length - 52))
      .readVarLengthBuffer().toString("hex");
  }

  // value of the output spent by this input
  get amount(): number {
    return this.getReader(
      this._buf.slice(this._buf.length - 44 - 8, this._buf.length - 44),
    ).readUInt32LE();
  }
  ...
  get nSequence(): number {
    return this.getReader(
      this._buf.slice(this._buf.length - 40 - 4, this._buf.length - 40),
    ).readUInt32LE();
  }

  // hashOutputs
  get hashOutputs(): string {
    return this._buf.slice(this._buf.length - 8 - 32, this._buf.length - 8)
      .toString("hex");
  }
  ...
  get nLocktime(): number {
    return this.getReader(
      this._buf.slice(this._buf.length - 4 - 4, this._buf.length - 4),
    ).readUInt32LE();
  }
  ...
  get sighashType(): number {
    return this.getReader(
      this._buf.slice(this._buf.length - 4, this._buf.length),
    ).readUInt32LE();
  }
  ...
  toLiteral(): string {
    return `SigHashPreimage(b'${
      getValidatedHexString(this._value.toString())
    }')`;
  }
...
export class Struct extends ScryptType {
  ...
  protected init(): void {
    checkStruct(Struct.getStructAst(this), this);
  ...
  getStructAst(): StructEntity {
    ...
    return {
      name: structAst.name,
      params: structAst.params.map((p) => ({
        name: p.name,
        type: resolveGenericType(this.inferredTypes, p.type),
      })),
      genericTypes: (structAst.genericTypes || []).map((t) => t),
    };
  }

  private inferrTypesByValue(): void {
    const value = this.value as StructObject;
    const structAst = this.getStructAst();
    structAst.params.forEach((p) => {
      const argType = inferrType(value[p.name]);
      if (structAst.genericTypes.includes(p.type)) {
        if (this.inferredTypes[p.type]) {
          if (this.inferredTypes[p.type] != argType) {
            throw new Error(
              `The type of ${structAst.name} is wrong, expected ${
                structSign(structAst)
  ...
  public inferrTypesByAssign(assignType: string): boolean {
    ...
    const [struct, genericTypes] = parseGenericType(assignType);
    ...
    let succces = true;
    structAst.genericTypes.forEach((t, index) => {
      ...
      if (v[key] instanceof ScryptType) {
        if (Struct.isStruct(v[key])) {
          ...
          return Object.assign(obj, {
            [key]: (v[key] as ScryptType).toLiteral(),
          });
        }
      } else {
  ...
  static hasGeneric(self: Struct): boolean {
    const ast: StructEntity = Struct.getStructAst(self) as StructEntity;
  ...
  get finalType(): string {
    if (this._typeResolver) {
      if (Struct.hasGeneric(this)) {
        const structAst = this.getStructAst();
        const type = toGenericType(
          this.type,
          structAst.genericTypes.map((t) => (this.inferredTypes[t] || t)),
        );
...
export class Library extends ScryptType {
  ...
  getProperty(property: string): SupportedParamType {
    ...
    if (typeof p === "undefined") {
      throw new Error(
        `Property ${property} not found, the properties of the current library are: ${
  ...
  checkValue(args: SupportedParamType[]): ValueType {
    ...
    if (libraryAst.params.length !== args.length) {
      throw new Error(
        `The type of ${libraryAst.name} is wrong, expected ${
          librarySign(libraryAst)
  ...
  attach() {
    ...
    libraryAst.params.forEach((param, index) => {
      const error = checkSupportedParamType(
        args[index],
        param,
        this._typeResolver,
      );
  ...
  get finalType(): string {
    if (this._typeResolver) {
      if (Library.hasGeneric(this)) {
        ...
        const type = toGenericType(
          this.type,
          libraryAst.genericTypes.map((t) => (this.inferredTypes[t] || t)),
        );
        return this._typeResolver(type).finalType;
      } else {
        return this._typeResolver(this.type).finalType;
...
export class HashedMap extends Library {
  constructor(data: Bytes) {
    ...
    this._typeResolver = (t: string) => ({
      finalType: t,
      symbolType: SymbolType.Library,
    }); //we should assign this before bind
    this.bind();
  }
}
...
export const BasicType = Object.keys(VariableType).map((key) =>
  VariableType[key]
);
