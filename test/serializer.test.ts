...
describe('serializer', () => {
  ...
  describe('deserializeState()', () => {
    ...
    it('special number', () => {
      ...
      expect(serial).to.equal('00 81 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f 10 11 12 2800')
      expect(hex).to.equal('516a01000181010101020103010401050106010701080109010a010b010c010d010e010f011001110112022800')
    ...
    it('special string', () => {
      const states = ['OP_0', 'OP_1', 'OP_2', 'OP_3', 'OP_4', 'OP_5', 'OP_6', 'OP_7', 'OP_8', 'OP_9', 'OP_10', 'OP_11', 'OP_12', 'OP_13', 'OP_14', 'OP_15', 'OP_16', 17, 18, 19]
      const serial = serializeState(states)
      const script = Script.fromASM('OP_TRUE OP_RETURN ' + serial)
      const hex = script.toHex()

      expect(serial).to.equal('0 OP_1 OP_2 OP_3 OP_4 OP_5 OP_6 OP_7 OP_8 OP_9 OP_10 OP_11 OP_12 OP_13 OP_14 OP_15 OP_16 11 12 13 1700')
      expect(hex).to.equal('516a005152535455565758595a5b5c5d5e5f60011101120113021700')
    ...
    it('pushdata 4', () => {
      ...
      expect(serial).to.equal('ff'.repeat(2 ** 16) + ' 05000100')
      expect(hex).to.equal('516a4e00000100' + 'ff'.repeat(2 ** 16) + '0405000100')
